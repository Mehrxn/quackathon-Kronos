...
class Orchestrator:
    ...
    async def _run_inner(self, incident: Incident, *, fetch_loki: bool) -> None:
        ...
        # Step 4: pull detailed logs (merge with any provided in webhook)
        ...
        if lookup.result == "exact":
            # Step: short-circuit deep diagnosis
            diagnosis = Diagnosis(
            ...
            # Steps 1-5 retrieval (Phases 2-5)
        ...
        # Steps 11/12
        if decision.action == Action.AUTO_FIX:
    ...
    async def _auto_fix_path(self, incident: Incident, diagnosis: Diagnosis) -> None:
        ...
        commit_msg = f"FIX ALERT {time.strftime('%Y-%m-%d %H:%M:%S')} Priority {prio}"
        self.github.commit_all(commit_msg)
        self.github.push_branch(branch)
        title = f"FIX ALERT {time.strftime('%Y-%m-%dT%H:%M:%S')} Priority {prio}"
        body = self._pr_body(incident, diagnosis)
        url = self.github.open_pr(branch=branch, title=title, body=body)
        incident.pr_url = url
