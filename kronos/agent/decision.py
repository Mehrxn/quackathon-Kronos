...
class DecisionEngine:
    ...
    def reconcile_priority(
        self, hint: Optional[Priority], diagnosis: Diagnosis
    ) -> tuple[Priority, str]:
        ...
        if diagnosis.confidence >= req:
            msg = (
                f"DISAGREEMENT hint={hint.value} llm={llm_prio.value}; "
    ...
    def decide(
        self,
        *,
        hint: Optional[Priority],
        diagnosis: Diagnosis,
        test_reproduced: Optional[bool],
    ) -> Decision:
        ...
        if resolved == Priority.HIGH:
            return Decision(
                resolved,
                Action.AUTO_FIX,
                "high priority always attempts fix",
                confidence_ok,
            )
        if resolved == Priority.LOW:
            return Decision(
                resolved,
                Action.ISSUE,
                "low priority is always issue-only",
                confidence_ok,
            )
        # medium
        if auto_fix == "follow_autonomy":
            if self.full_autonomous:
                return Decision(
                    resolved,
                    Action.AUTO_FIX,
                    "medium + full_autonomous -> fix",
                    confidence_ok,
                )
            return Decision(
                resolved,
                Action.ISSUE,
                "medium + not autonomous -> issue",
                confidence_ok,
            )
        if auto_fix is True:
            return Decision(
                resolved, Action.AUTO_FIX, "medium auto_fix=true", confidence_ok
            )
        return Decision(resolved, Action.ISSUE, "medium auto_fix=false", confidence_ok)
