...
const AGENT_URL = process.env.AGENT_URL || 'http://kronos:8001/api/v1/init/';
...
function determineLogQLQuery(alert) {
  ...
  if (
    ...
    alertName.includes('vcpu') ||
    description.includes('cpu') ||
    description.includes('vcpu') ||
    summary.includes('cpu') ||
    summary.includes('vcpu')
  ) {
  ...
  if (
    alertName.includes('segfault') ||
    description.includes('segfault') ||
    summary.includes('segfault') ||
    alertName.includes('sigsegv') ||
    description.includes('sigsegv') ||
    summary.includes('sigsegv')
  ) {
    console.log(
      `🔍 [Loki Filter] Identified Segfault crash alert. Applying crash/segfault filters.`
    );
  ...
  if (
    alertName.includes('outofindex') ||
    alertName.includes('index') ||
    description.includes('outofindex') ||
    description.includes('index') ||
    summary.includes('outofindex') ||
    ...
    .get(lokiEndpoint, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log(
            `✅ [Loki Sync] Successfully retrieved ${collectedLogLines.length} contextual logs.`
          );
  ...
  const req = http.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 422) {
        console.error(
          '⚠️  [AI Pipeline] 422 from Kronos — InitRequest shape mismatch. ' +
...
server.listen(PORT, () =>
  console.log(`🚀 Webhook processor listening for Grafana Alerts on port ${PORT}`)
);
